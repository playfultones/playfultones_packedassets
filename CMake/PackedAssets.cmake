...
function(_pt_pa_ensure_packer)
    ...
    # Build the packer from a generated unity TU rather than listing the module's
    # own source files directly. When a consumer juce_add_module()s this module
    # with source groups enabled (JUCE_ENABLE_MODULE_SOURCE_GROUPS), JUCE marks
    # every non-umbrella module file HEADER_FILE_ONLY in the consumer's directory
    # scope -- and re-applies that on every subsequent juce_add_* call. Listing
    # those files as the packer's sources leaves it with nothing to compile (it
    # links with zero objects and fails). A unity file under the build dir is
    # outside the module tree, so JUCE never touches it; it #includes the real
    # sources from their own locations, so their relative includes still resolve
    # (this mirrors the module's own umbrella .cpp).
    set(_packer_gen ${CMAKE_BINARY_DIR}/playfultones_packedassets_gen)
    file(MAKE_DIRECTORY ${_packer_gen})
    set(_packer_unity ${_packer_gen}/pt_pak_packer_unity.cpp)
    file(WRITE ${_packer_unity}
        "// Generated by playfultones_packedassets_add_pack(). Do not edit.\n"
        "extern \"C\" {\n"
        "#include \"${PT_PACKEDASSETS_DIR}/third_party/tiny-aes/aes.c\"\n"
        "}\n"
        "#include \"${PT_PACKEDASSETS_DIR}/src/Crypto.cpp\"\n"
        "#include \"${PT_PACKEDASSETS_DIR}/src/Packer.cpp\"\n"
        "#include \"${PT_PACKEDASSETS_DIR}/tools/pak_packer.cpp\"\n")
    add_executable(pt_pak_packer ${_packer_unity})
